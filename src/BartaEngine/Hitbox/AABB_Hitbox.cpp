...
bool Barta::AABB_Hitbox::isWithin( const Vector2f & position ) const{
	return this->getAABB().getLeftTop().getX() <= position.getX()
		&& position.getX() <= this->getAABB().getRightBottom().getX()
		&& this->getAABB().getLeftTop().getY() <= position.getY()
		&& position.getY() <= this->getAABB().getRightBottom().getY();
}

Barta::CollisionTestResult Barta::AABB_Hitbox::intersects( const HitboxInterface& secondHitbox, const CollisionDetectionStrategyInterface& collisionDetector, const DynamicsDTO& dynamicsDifference ) const{
	return secondHitbox.intersectsWithAABB(
		this->getAABB(),
		collisionDetector,
		dynamicsDifference
	);
...
std::unique_ptr<const Barta::HitboxInterface> Barta::AABB_Hitbox::getTransformedHitbox( const TransformableInterface& transformable ) const{
